...
Available macros:
  ...
  every_n_seconds   — run a one-shot every N seconds, forever (until parent
                      terminates)
  timeout_wrap      — wrap an action block in an exception+heartbeat
                      timeout, with optional on_timeout one-shot
  guarded_action    — run a one-shot only if a predicate boolean returns True
  wait_then_act     — wait for N events of a given id, then fire a one-shot
