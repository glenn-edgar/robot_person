...
-- Mirrors test_dispatch.c (advanced_primitive_test)
-- ============================================================================
...
local function result_to_str(r)
    ...
    -- Function codes (6-11)
    elseif r == SE.SE_FUNCTION_CONTINUE      then return "FUNCTION_CONTINUE"
    elseif r == SE.SE_FUNCTION_HALT          then return "FUNCTION_HALT"
    elseif r == SE.SE_FUNCTION_TERMINATE     then return "FUNCTION_TERMINATE"
    elseif r == 9                            then return "FUNCTION_RESET"
    elseif r == SE.SE_FUNCTION_DISABLE       then return "FUNCTION_DISABLE"
    elseif r == 11                           then return "FUNCTION_SKIP_CONTINUE"
    -- Pipeline codes (12-17)
    elseif r == SE.SE_PIPELINE_CONTINUE      then return "PIPELINE_CONTINUE"
    elseif r == SE.SE_PIPELINE_HALT          then return "PIPELINE_HALT"
    elseif r == SE.SE_PIPELINE_TERMINATE     then return "PIPELINE_TERMINATE"
    elseif r == SE.SE_PIPELINE_RESET         then return "PIPELINE_RESET"
    elseif r == SE.SE_PIPELINE_DISABLE       then return "PIPELINE_DISABLE"
    elseif r == SE.SE_PIPELINE_SKIP_CONTINUE then return "PIPELINE_SKIP_CONTINUE"
...
local function result_is_terminate(r)
    local SE = se_runtime
    return r == 2                        -- SE_TERMINATE (app)
        or r == SE.SE_FUNCTION_TERMINATE
        or r == SE.SE_PIPELINE_TERMINATE
end

local function result_is_complete(r)
    local SE = se_runtime
    return r == 2                        -- SE_TERMINATE
        or r == 4                        -- SE_DISABLE
        or r == SE.SE_FUNCTION_TERMINATE
        or r == SE.SE_FUNCTION_DISABLE
        or r == SE.SE_PIPELINE_TERMINATE
        or r == SE.SE_PIPELINE_DISABLE
end
...
-- Mirrors display_event_info() in advanced_primitive_test_user_functions.c
--
-- In C: declared as a void oneshot but receives full event context
-- (event_type, event_id, event_data). Skips silently on TICK events.
-- event_data is cast to uint16_t byte offset into blackboard to read a float.
--
-- In Lua: registered as an ONESHOT (fn(inst, node) — no event args).
-- Event context is read from inst.current_event_id / inst.current_event_data,
-- which the runtime sets before every invocation.
-- event_data is the blackboard field name string queued by se_queue_event.
...
-- ============================================================================
-- LOAD MODULE AND BUILD FUNCTION TABLE
-- ============================================================================

local ok, md = pcall(require, "advanced_primitive_test_module")
if not ok then
    print("❌ FATAL: Could not load advanced_primitive_test_module: " .. tostring(md))
    os.exit(1)
end
...
-- Merge all builtin tables plus user functions
local fns = se_runtime.merge_fns(
    ...
    {
        -- Registered under both likely names; register_fns silently ignores
        -- whichever doesn't match the module (case-insensitive lookup via upper())
...
local function test_dispatch(mod, md, tree_hash, tree_label)
    ...
    repeat
        ...
        while event_count > 0 do
            ...
            -- Save tick_type, dispatch, restore (mirrors C)
...
-- Run against DISPATCH_TEST_HASH (first tree if hash lookup fails)
