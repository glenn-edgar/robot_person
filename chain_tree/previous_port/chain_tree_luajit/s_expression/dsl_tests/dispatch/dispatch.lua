...
local mod = start_module("dispatch_test")
use_32bit()
set_debug(true)
...
field_actions_fn[1] = function() 
    se_case(1, function()
        se_chain_flow(function()
            se_log("field_actions_fn[1]")
            se_tick_delay(5)
            se_log("field_actions_fn[1] terminated")
            se_return_pipeline_reset()
        end)
    end) 
end

field_actions_fn[2] = function() 
    se_case(2 , function()
        se_chain_flow(function()
            se_log("field_actions_fn[2]")
            se_tick_delay(5)
            se_log("field_actions_fn[2] terminated")
            se_return_pipeline_reset()
        end)
    end) 
end

field_actions_fn[3] = function() 
    se_case('default', function()
        se_chain_flow(function()
            se_log("field_actions_fn[3]")
            se_tick_delay(5)
            se_log("field_actions_fn[3] terminated")
            se_return_pipeline_reset()
        end)
    end) 
end
...
start_tree("dispatch_test")
    ...
    se_function_interface(function()
        ...
        se_log("State machine test started ")
         ...
         se_field_dispatch("state", field_actions_fn)
