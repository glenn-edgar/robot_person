-- ============================================================================
-- main_stack_equations_test.lua
-- LuaJIT test driver for stack_equations
-- Mirrors stack_equations C driver (test_dispatch / main)
-- No user functions — stack_equations_register_all is empty in C.
-- ============================================================================
...
-- ============================================================================
-- LOAD MODULE
-- No user functions — C register block is empty.
-- ============================================================================

local ok, md = pcall(require, "stack_equations_module")
if not ok then
    print("❌ FATAL: Could not load stack_equations_module: " .. tostring(md))
    os.exit(1)
end
...
local function test_dispatch(mod, md)
    print("")
    print("╔════════════════════════════════════════╗")
    print("║    STACK EQUATIONS TEST                ║")
    print("╚════════════════════════════════════════╝")
    print("")
    print("Testing stack equations with tick loop...")
    ...
    -- Mirrors s_expr_tree_create_stack(tree, 128)
    inst.stack = require("se_stack").new_stack(128)
    ...
    repeat
        -- Reset stack top before each tick (stack is single-tick scoped)
        if inst.stack then
            inst.stack.sp          = 0
        ...
        -- Process queued events (mirrors C event loop)
